...
const updatePage = async () => {
  ...
  const someFruit = fruitsArray.filter((item) => {
    return item.id > 6;
  })

  const fruitNames = someFruit.map((item) => {
    return item.name;
  })
  ...
  newElement.innerHTML = fruitNames;

  const galleryDiv = document.getElementById("pinkBox");
  galleryDiv.append(newElement);
